...
export const fetchAllRoMooc = (page = 1, limit = 50) => {
  return mockApiCall(() => 
    withPagination(() => roMoocDataService.getAllRoMooc(), { page, limit })
  );
};
...
export const editRoMooc = (id, data) => {
  return mockApiCall(() => 
    withUpdate(() => roMoocDataService.updateRoMooc(id, data), 'Ro mooc not found', 'Ro mooc updated successfully')
  );
};

export const removeRoMooc = id => {
  return mockApiCall(() => 
    withDelete(() => roMoocDataService.deleteRoMooc(id), 'Ro mooc not found', 'Ro mooc deleted successfully')
  );
};
