...
const generateSampleLichVanChuyen = (count = 15) => {
  ...
  for (let i = 1; i <= count; i++) {
    ...
    const newRecord = {
      ...
      ghi_chu: trang_thai === TRANG_THAI_LICH_VAN_CHUYEN.HUY_BO ? 'Lý do hủy: Thời tiết xấu' : (i % 4 === 0 ? 'Hàng dễ vỡ, xin nhẹ tay' : ''),
...
export const createLichVanChuyen = async (data) => {
  const { ma_chuyen, ngay_van_chuyen, trang_thai, khach_hang_id, diem_xuat_phat, diem_tra_hang, bien_so_xe_id, container_id, nhan_vien_giao_nhan_id, nhan_vien_lai_xe_id } = data;

  if (!ma_chuyen || !ngay_van_chuyen || !trang_thai || !khach_hang_id || !diem_xuat_phat || !diem_tra_hang || !bien_so_xe_id || !container_id || !nhan_vien_giao_nhan_id || !nhan_vien_lai_xe_id) {
  ...
  if (!Object.values(TRANG_THAI_LICH_VAN_CHUYEN).includes(trang_thai)) {
    ...
    throw new Error(`Invalid trang_thai value. Must be one of: ${Object.values(TRANG_THAI_LICH_VAN_CHUYEN).join(', ')}`);
  }
...
export const updateLichVanChuyen = async (id, updates) => {
  ...
  if (validUpdates.trang_thai && !Object.values(TRANG_THAI_LICH_VAN_CHUYEN).includes(validUpdates.trang_thai)) {
...
export const _resetLichVanChuyen = (data = []) => {
  ...
  const postResetDuplicateMaChuyen = currentMaChuyen.filter((item, index) => currentMaChuyen.indexOf(item) !== index);
  if (postResetDuplicateMaChuyen.length > 0) {
      console.error("CRITICAL: Duplicate ma_chuyen found after _resetLichVanChuyen:", postResetDuplicateMaChuyen);
  }
};

// Initial check for duplicate ma_chuyen in the seed data
const initialMaChuyen = lichVanChuyenData.map(c => c.ma_chuyen);
const duplicateMaChuyen = initialMaChuyen.filter((item, index) => initialMaChuyen.indexOf(item) !== index);
if (duplicateMaChuyen.length > 0) {
    console.error("CRITICAL: Duplicate ma_chuyen found in initial lichVanChuyenData:", duplicateMaChuyen);
}
