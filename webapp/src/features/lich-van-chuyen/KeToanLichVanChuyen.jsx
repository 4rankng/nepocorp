...
const EditIcon = ({ className = "w-4 h-4" }) => ( // Smaller for modal list
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
  </svg>
);
const DeleteIcon = ({ className = "w-4 h-4" }) => ( // Smaller for modal list
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
    <path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12.56 0c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
  </svg>
);
const PencilIcon = ({ className = "w-5 h-5" }) => ( // Main table edit icon
  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
...
const KeToanLichVanChuyen = () => {
  ...
  const renderEditableCell = (plan, fieldKey, displayValue, isCurrency = false) => {
    if (editingCell && editingCell.planId === plan.id && editingCell.field === fieldKey) {
      return (
        <input type="number" value={editValue} onChange={handleEditInputChange} onBlur={handleEditCommit} onKeyDown={handleEditKeyDown}
          className="w-full px-1 py-0.5 border border-blue-500 rounded-sm text-sm text-right" autoFocus min="0" />
      );
    }
  ...
  const openOtherCostsModal = (plan) => {
    ...
    setModalDetailedCosts(plan.detailedOtherCosts ? JSON.parse(JSON.stringify(plan.detailedOtherCosts)) : []);
    setNewCostItemName('');
  ...
  const handleModalCostItemChange = (index, field, value) => {
    ...
    updatedCosts[index] = { ...updatedCosts[index], [field]: field === 'amount' ? parseFloat(value) || 0 : value };
    setModalDetailedCosts(updatedCosts);
  };
  
  const handleAddOrUpdateModalCostItem = () => {
    if (!newCostItemName.trim() || !newCostItemAmount.trim() || parseFloat(newCostItemAmount) <= 0) {
      setError("Tên và số tiền hợp lệ là bắt buộc cho khoản chi.");
      return;
    }
    const amount = parseFloat(newCostItemAmount);
    if (editingCostItemId) { // Update existing item in modal
      setModalDetailedCosts(modalDetailedCosts.map(item => 
        item.id === editingCostItemId ? { ...item, name: newCostItemName.trim(), amount } : item
      ));
    } else { // Add new item to modal
      setModalDetailedCosts([...modalDetailedCosts, { id: `temp-${Date.now()}`, name: newCostItemName.trim(), amount }]);
    }
    setNewCostItemName('');
    setNewCostItemAmount('');
    setEditingCostItemId(null);
  ...
  const handleSaveOtherCosts = async () => {
    ...
    try {
      ...
      const itemsToAdd = modalDetailedCosts.filter(item => !item.id.startsWith('temp-') && !originalCosts.find(oc => oc.id === item.id) || item.id.startsWith('temp-') ); // New items (temp or new from DB if IDs were fetched)
      const itemsToUpdate = modalDetailedCosts.filter(item => !item.id.startsWith('temp-') && originalCosts.find(oc => oc.id === item.id && (oc.name !== item.name || oc.amount !== item.amount)));
      const itemsToDelete = originalCosts.filter(oc => !modalDetailedCosts.find(item => item.id === oc.id));

      for (const item of itemsToDelete) {
        await deleteDetailedOtherCostItem(selectedPlanForOtherCosts.id, item.id);
      }
      for (const item of itemsToUpdate) {
        await updateDetailedOtherCostItem(selectedPlanForOtherCosts.id, item.id, item.name, item.amount);
      }
      for (const item of itemsToAdd) { // Items with temp-id are definitely new
  ...
  return (
    <div className="p-4 md:p-6 bg-gray-100 min-h-screen">
      <div className="flex justify-between items-center mb-6 md:mb-8">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Sổ Kế Toán - Lịch Vận Chuyển</h1>
        <button onClick={handleAddNewPlan} /* ... */ > {/* ... */} </button>
      </div>

      {/* Error display for main page */}
      {error && !isOtherCostsModalOpen && <div className="mb-4 text-center text-red-500 bg-red-100 p-3 rounded-md">{error}</div>}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 transition-opacity">
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-xl max-h-[80vh] flex flex-col">
            <h2 className="text-xl font-semibold mb-4 text-gray-800">
              Chi Tiết Chi Phí Khác cho: <span className="font-normal">{selectedPlanForOtherCosts.dienGiai} ({selectedPlanForOtherCosts.ngayThang})</span>
            </h2>
            ...
            <div className="flex-grow overflow-y-auto pr-2">
              {modalDetailedCosts.length === 0 && <p className="text-gray-500 text-sm">Chưa có khoản chi phí khác nào.</p>}
              {modalDetailedCosts.map((item, index) => (
                <div key={item.id || `new-${index}`} className="flex items-center space-x-2 py-2 border-b last:border-b-0">
                   {editingCostItemId === item.id ? (
                    <>
                      <input type="text" value={newCostItemName} onChange={(e) => setNewCostItemName(e.target.value)} placeholder="Tên khoản chi" className="flex-grow input-style text-sm p-1"/>
                      <input type="number" value={newCostItemAmount} onChange={(e) => setNewCostItemAmount(e.target.value)} placeholder="Số tiền" className="w-32 input-style text-sm p-1"/>
                    </>
                  ) : (
                    <>
                      <span className="flex-grow text-sm text-gray-700">{item.name}</span>
                      <span className="w-32 text-sm text-gray-700 text-right">{formatCurrency(item.amount)}</span>
                    </>
                  )}
                  {editingCostItemId === item.id ? (
                     <button onClick={handleAddOrUpdateModalCostItem} className="text-green-500 hover:text-green-700 p-1"><PlusCircleIcon className="w-5 h-5"/></button>
                  ) : (
                     <button onClick={() => handleEditModalCostItem(item)} className="text-blue-500 hover:text-blue-700 p-1"><EditIcon /></button>
                  )}
                  <button onClick={() => handleDeleteModalCostItem(item.id)} className="text-red-500 hover:text-red-700 p-1"><DeleteIcon /></button>
                </div>
              ))}
            </div>
            
            <div className="mt-4 pt-4 border-t">
              <h3 className="text-md font-semibold mb-2 text-gray-700">{editingCostItemId ? 'Cập nhật Khoản Chi' : 'Thêm Khoản Chi Mới'}</h3>
              <div className="flex items-center space-x-2 mb-4">
                <input type="text" value={newCostItemName} onChange={(e) => setNewCostItemName(e.target.value)} placeholder="Tên khoản chi" className="flex-grow input-style p-2"/>
                <input type="number" value={newCostItemAmount} onChange={(e) => setNewCostItemAmount(e.target.value)} placeholder="Số tiền" className="w-40 input-style p-2"/>
                <button onClick={handleAddOrUpdateModalCostItem} className="bg-green-500 hover:bg-green-600 text-white px-3 py-2 rounded-md text-sm">
                  {editingCostItemId ? 'Cập nhật' : 'Thêm'}
                </button>
                {editingCostItemId && <button onClick={() => { setEditingCostItemId(null); setNewCostItemName(''); setNewCostItemAmount('');}} className="text-gray-500 hover:text-gray-700 text-sm">Hủy Sửa</button>}
              </div>
            </div>

            <div className="mt-6 flex justify-end space-x-3">
              <button onClick={closeOtherCostsModal} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-md">Hủy</button>
              <button onClick={handleSaveOtherCosts} disabled={isLoading} className={`px-4 py-2 text-sm font-medium text-white rounded-md ${isLoading ? 'bg-blue-300 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}>
