...
const QuanLyPhieuThu = () => {
  ...
  const handleEditInvoice = useCallback(async (invoice) => {
    try {
      ...
      showSnackbar('Đang tải thông tin hóa đơn...', 'info');
      ...
      showSnackbar('Không thể tải thông tin hóa đơn', 'error');
    }
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      ...
      showSnackbar('Xóa hóa đơn thành công', 'success');
      handleDeleteClose();
    } catch (error) {
      showSnackbar('Không thể xóa hóa đơn', 'error');
    } finally {
    ...
    <div className="p-6">
      ...
      {showInvoiceForm && (
        <InvoiceForm
          ...
          title={editingInvoice ? 'Sửa hóa đơn' : 'Thêm hóa đơn mới'}
        />
      )}
