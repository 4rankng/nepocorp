...
const QuanLyDinhMucDuong = () => {
  ...
  const fetchCostRatesData = async () => {
    setIsLoading(true);
    setError('');
    try {
      const data = await getCostRates();
      setCostRates(data);
    } catch (err) {
      setError('Không thể tải danh sách định mức.');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleOpenModalForEdit = rate => {
    ...
    setFormData({
      ...
      kmMin: String(rate.kmMin),
      kmMax: String(rate.kmMax),
  ...
  const handleSaveRate = async () => {
    ...
    const kmMinValue = parseFloat(kmMin);
    const kmMaxValue = parseFloat(kmMax);
    ...
    try {
      if (editingRate) {
        await updateCostRate(editingRate.id, rateDataPayload);
      } else {
        await addCostRate(rateDataPayload);
      }
      await fetchCostRatesData();
      handleCloseModal();
    } catch (err) {
      setError(err.message || `Lỗi khi ${editingRate ? 'cập nhật' : 'thêm'} định mức.`);
      console.error(err);
    } finally {
      setIsLoading(false);
    }
...
export default QuanLyDinhMucDuong;
