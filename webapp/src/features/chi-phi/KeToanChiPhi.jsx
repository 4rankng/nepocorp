...
const KeToanChiPhi = () => {
  ...
  const [newGeneralExpenseFormData, setNewGeneralExpenseFormData] = useState(initialGeneralExpenseFormState);
  ...
  const handleAddGeneralExpense = async () => { 
    ...
    const expensePayload = {
      ...
      category: newGeneralExpenseFormData.category === 'Khác' ? newGeneralExpenseFormData.customCategory.trim() : newGeneralExpenseFormData.category,
    ...
    if (!expensePayload.date || !expensePayload.category || !expensePayload.description || isNaN(expensePayload.amount) || expensePayload.amount <= 0) {
  ...
  const handleAddTireExpense = async () => {
    ...
    if (!tirePayload.vehicleId || !tirePayload.purchaseDate || !tirePayload.brand || !tirePayload.serialNumber || 
        !tirePayload.size || isNaN(tirePayload.quantity) || tirePayload.quantity <= 0 ||
        isNaN(tirePayload.unitPrice) || tirePayload.unitPrice <= 0 || !tirePayload.supplier) {
        setError("Vui lòng điền đầy đủ các trường bắt buộc cho chi phí lốp và đảm bảo số lượng, đơn giá hợp lệ.");
        return;
    }
  ...
  return (
    <div className="p-4 md:p-6 bg-white min-h-screen"> {/* Ensured white background */}
      ...
      <div className="mb-6 p-4 bg-gray-50 shadow rounded-lg">
         <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div>
            <label htmlFor="vehicleIdFilter" className="block text-sm font-medium text-gray-700 mb-1">Lọc theo xe</label>
            <select id="vehicleIdFilter" name="vehicleId" value={filters.vehicleId} onChange={handleFilterChange} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md" disabled={isLoadingDropdowns}>
              <option value="">Tất cả xe</option>
              {vehiclesForSelect.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}
            </select>
          </div>
          <div>
            <label htmlFor="monthYearFilter" className="block text-sm font-medium text-gray-700 mb-1">Lọc theo tháng</label>
            <select id="monthYearFilter" name="monthYear" value={filters.monthYear} onChange={handleFilterChange} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm rounded-md" disabled={isLoadingDropdowns}>
      ...
      <div className="mb-8 p-4 bg-white shadow-sm border border-gray-200 rounded-lg">
        ...
        {isLoadingChartData && <div className="text-center text-gray-500 py-4">Đang tải biểu đồ...</div>}
        {!isLoadingChartData && chartError && <div className="text-center text-red-600 bg-red-50 p-3 rounded-md">{chartError}</div>}
        {!isLoadingChartData && !chartError && aggregatedChartData.length === 0 && (
          <div className="text-center text-gray-500 py-4">Không có dữ liệu chi phí tổng hợp cho lựa chọn này.</div>
        )}
        {!isLoadingChartData && !chartError && aggregatedChartData.length > 0 && (
            ...
            {aggregatedChartData.map((item) => (
              <div key={item.category} className="py-1 border-b border-gray-100 last:border-b-0">
                <div className="flex justify-between items-center text-sm mb-0.5">
                  <span className="font-medium text-gray-700 w-2/5 truncate" title={item.category}>{item.category}</span>
                  <span className="font-semibold text-gray-800 w-1/5 text-right">{formatCurrency(item.totalAmount)}</span>
                </div>
                <div className="w-full bg-gray-200 rounded-full h-5 border border-gray-300 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${maxChartBarValue > 0 ? (item.totalAmount / maxChartBarValue) * 100 : 0}%` }}
      ...
      <div className="flex justify-between items-center mb-4 mt-8 pt-4 border-t">
        ...
        <button onClick={handleOpenGeneralModal} className="flex items-center bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-3 rounded-lg shadow-md text-sm">
          <PlusIcon className="w-5 h-5 mr-1" /> Thêm Chi Phí Chung
        </button>
      </div>
      {error && !isGeneralModalOpen && !isTireModalOpen && <div className="mb-4 text-center text-red-500 bg-red-100 p-3 rounded-md">{error}</div>}
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {isLoadingGeneralExpenses ? (
              <tr><td colSpan="5" className="p-4 text-center text-gray-500">Đang tải chi phí chung...</td></tr>
            ) : expensesList.length === 0 && !error ? (
              <tr><td colSpan="5" className="p-4 text-center text-gray-500">Không có chi phí chung nào phù hợp.</td></tr>
            ) : (
              expensesList.map((expense) => (
                <tr key={expense.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{formatDateForDisplay(expense.date)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{expense.bienSoXe || '-'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{expense.category}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 max-w-md truncate" title={expense.description}>{expense.description}</td>
      ...
      <div className="mt-10 pt-6 border-t">
         <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={handleOpenTireModal} className="flex items-center bg-green-600 hover:bg-green-700 text-white font-medium py-2 px-3 rounded-lg shadow-md text-sm">
         ...
         <div className="mb-6 p-4 bg-gray-50 shadow rounded-lg">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                <div>
                    <label htmlFor="tireVehicleFilter" className="block text-sm font-medium text-gray-700 mb-1">Lọc theo xe (Lốp xe)</label>
                    <select id="tireVehicleFilter" name="tireVehicleFilter" value={tireVehicleFilter} onChange={handleTireFilterChange} className="mt-1 block w-full select-style" disabled={isLoadingDropdowns}>
                        <option value="">Tất cả xe</option>
                        {vehiclesForSelect.map(v => <option key={v.id} value={v.id}>{v.name}</option>)}
                    </select>
                </div>
        ...
        <div className="bg-white shadow-sm border border-gray-200 rounded-lg overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
           <thead className="bg-gray-50">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ngày Mua</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Biển Số Xe</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nhãn Hiệu</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Số Series</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Cỡ Lốp</th>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ngày Thay</th>
                <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Tuổi Thọ (ngày)</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {isLoadingTires ? (
                <tr><td colSpan="11" className="p-4 text-center text-gray-500">Đang tải chi phí lốp...</td></tr>
              ) : tireExpensesList.length === 0 && !chartError && !error && !isLoadingGeneralExpenses ? ( 
                <tr><td colSpan="11" className="p-4 text-center text-gray-500">Không có chi phí lốp nào.</td></tr>
              ) : (
                tireExpensesList.map(tire => (
                  <tr key={tire.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 whitespace-nowrap text-sm">{formatDateForDisplay(tire.purchaseDate)}</td>
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-right">{tire.quantity}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-right">{formatCurrency(tire.unitPrice)}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-right">{formatCurrency(tire.totalPrice)}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm">{tire.supplier}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm">{formatDateForDisplay(tire.replacementDate) || '-'}</td>
                    <td className="px-4 py-3 whitespace-nowrap text-sm text-right">{tire.tireAgeInDays}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-lg transform transition-all max-h-[90vh] flex flex-col">
            ...
            <div className="mt-6 flex justify-end space-x-3 pt-4 border-t">
              <button type="button" onClick={handleCloseGeneralModal} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-md">Hủy</button>
              <button type="button" onClick={handleAddGeneralExpense} disabled={isLoadingGeneralExpenses} className={`px-4 py-2 text-sm font-medium text-white rounded-md ${isLoadingGeneralExpenses ? 'bg-blue-300 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}>
      ...
      {isTireModalOpen && (
         <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
            ...
            <div className="mt-6 flex justify-end space-x-3 pt-4 border-t">
              <button type="button" onClick={handleCloseTireModal} className="px-4 py-2 text-sm font-medium text-gray-700 bg-gray-100 hover:bg-gray-200 rounded-md">Hủy</button>
              <button type="button" onClick={handleAddTireExpense} disabled={isLoadingTires} className={`px-4 py-2 text-sm font-medium text-white rounded-md ${isLoadingTires ? 'bg-green-300 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'}`}>
