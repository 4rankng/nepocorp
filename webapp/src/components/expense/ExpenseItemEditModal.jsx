...
import { calculateItemTotal } from '@utils/expenseHelpers';
...
const ExpenseItemEditModal = ({
    ...
    if (isOpen) {
      if (isEdit && item) {
        setFormData({
          ...
          install_date: item.install_date ? item.install_date.split('T')[0] : null,
  ...
  useEffect(() => {
    const calculatedTotal = calculateItemTotal(formData);
  ...
  const validateForm = useCallback(() => {
    ...
    if (!formData.item_name?.trim()) {
      newErrors.item_name = 'Vui lòng nhập tên hạng mục';
    }
  ...
  return (
    <>
      <FormModal
        ...
        title={isEdit ? 'Sửa hạng mục' : 'Thêm hạng mục mới'}
        ...
        actions={
          <FormActionButtons
            ...
            loading={isSubmitting}
            cancelText="Hủy"
            submitText={isEdit ? 'Lưu' : 'Thêm hạng mục'}
          />
        }
      >
...
ExpenseItemEditModal.propTypes = {
