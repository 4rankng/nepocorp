...
import { MaintenanceFilters, CreateMaintenanceRequest, UpdateMaintenanceRequest } from '@api/types';

// Maintenance queries
export const useMaintenance = (
  filters?: MaintenanceFilters & { page?: number; limit?: number }
) => {
  ...
  useEffect(() => {
    ...
    const fetchMaintenanceByLicensePlate = async () => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err : new Error('Failed to fetch maintenance by license plate')
        );
      } finally {
        setIsLoading(false);
      }
...
export const useMaintenanceByDateRange = (
  startDate: string,
  endDate: string,
  page = 1,
  limit = 100
  ...
  useEffect(() => {
    ...
    const fetchMaintenanceByDateRange = async () => {
      ...
      try {
        ...
        setError(
          err instanceof Error ? err : new Error('Failed to fetch maintenance by date range')
        );
      } finally {
        setIsLoading(false);
      }
...
export const useUpdateMaintenance = () => {
  ...
  const mutate = useCallback(
    async ({ id, data }: { id: number; data: UpdateMaintenanceRequest }) => {
      setIsLoading(true);
      setError(null);
      try {
