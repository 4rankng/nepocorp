  ...
  return (
    ...
    <CrudTable<CapTableHistory>
      ...
      toolbarLeft={({ totalItems, activeCount }) => activeCount > 0 ? (
        <span style={{ fontSize: 'var(--fs-body)', color: 'var(--fg-3)' }}>
      ...
      columns={[
        {
          ...
          render: (ct, _i, isActive) => (
            <span style={{ fontWeight: 600, color: 'var(--fg-1)' }}>
              {ct.partnerName}
              {isActive && (
                <span style={{ marginLeft: 8, padding: '1px 7px', borderRadius: 999, background: 'var(--accent-soft)', color: 'var(--accent)', fontSize: 'var(--fs-body)', fontWeight: 700, letterSpacing: '0.04em' }}>HIỆN TẠI</span>
              )}
            </span>
          ),
        },
