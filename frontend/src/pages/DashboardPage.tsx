...
export default function DashboardPage() {
  ...
  return (
    <div className="fade-up" style={styles.sectionPadding}>
      <header className="page-header">
        <div>
          <h1 className="page-title">{(() => { const h = new Date().getHours(); return h < 12 ? 'Chào buổi sáng' : h < 18 ? 'Chào buổi chiều' : 'Chào buổi tối'; })()}, <span style={{ fontWeight: 700, fontStyle: 'normal', whiteSpace: 'nowrap' }}>{user?.fullName || (user?.role && ROLE_LABELS[user.role as Role]) || user?.username || 'bạn'}</span></h1>
