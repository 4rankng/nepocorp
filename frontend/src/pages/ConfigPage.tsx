...
export default function ConfigPage() {
  ...
  return (
    <div ref={rootRef}>
      ...
      {searchQuery.trim() && (
        <div style={{ marginBottom: 16, fontSize: 'var(--fs-body)', color: 'var(--fg-3)', display: 'flex', alignItems: 'center', gap: 6 }}>
          <Search size={14} />
      ...
      {filteredCards.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '64px 32px', border: '1px dashed var(--line)', borderRadius: 12, background: 'var(--bg-2)', color: 'var(--fg-3)' }}>
          ...
          <h3 style={{ fontSize: 'var(--fs-section)', fontWeight: 600, color: 'var(--fg-2)', marginBottom: 4 }}>Không tìm thấy cấu hình</h3>
          <p style={{ fontSize: 'var(--fs-body)' }}>Hãy thử tìm kiếm với từ khóa khác.</p>
        </div>
      ) : (
