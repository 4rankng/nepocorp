...
export function TripChecklistPanel() {
  ...
  const { mainInfo, journey, fuelRevenue, fuelRevenueTotal, images, imagesTotal } = form.completionStatus;
  ...
  const items = [
    ...
    { name: 'Nhiên liệu & doanh thu', done: fuelRevenue >= 5, active: fuelRevenue > 0, badge: `${fuelRevenue}/${fuelRevenueTotal}`, badgeClass: fuelRevenue >= 5 },
    { name: 'Hình ảnh & ghi chú', done: images >= 1, active: images > 0, badge: images === 0 ? '—' : `${images}/${imagesTotal}`, badgeClass: images >= 1 },
  ];
