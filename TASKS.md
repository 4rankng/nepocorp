...
- [ ] **T-B.2.1:** Add overdue summary endpoint: `GET /reports/receivables-summary` returning counts/amounts for 30/60/90+ day buckets
...
- [ ] **T-E.1.1:** Fix snake_case/camelCase property mismatches between backend API responses and frontend types (e.g., `entityType` vs `entity_type`, `distance` vs `distance_km`)
- [ ] **T-E.1.2:** Align frontend TypeScript interfaces with shared package types from `shared/src/types/index.ts`
- [ ] **T-E.1.3:** Run `cd frontend && npx tsc --noEmit` and fix all errors to zero
