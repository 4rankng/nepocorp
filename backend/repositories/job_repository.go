...
func (r *JobRepository) GetJobByID(id uint) (*models.Job, error) {
	var job models.Job
	err := r.DB.Preload("Tractor").Preload("Trailer").Preload("Driver").Preload("Customer").First(&job, id).Error
	return &job, err
}

func (r *JobRepository) GetAllJobs() ([]models.Job, error) {
	var jobs []models.Job
	err := r.DB.Preload("Tractor").Preload("Trailer").Preload("Driver").Preload("Customer").Find(&jobs).Error
	return jobs, err
}
...
func (r *JobRepository) GetJobsByTractorID(tractorID uint) ([]models.Job, error) {
	var jobs []models.Job
	err := r.DB.Where("tractor_id = ?", tractorID).Preload("Tractor").Preload("Trailer").Preload("Driver").Preload("Customer").Find(&jobs).Error
	return jobs, err
}

func (r *JobRepository) GetJobsByCustomerID(customerID uint) ([]models.Job, error) {
	var jobs []models.Job
	err := r.DB.Where("customer_id = ?", customerID).Preload("Tractor").Preload("Trailer").Preload("Driver").Preload("Customer").Find(&jobs).Error
	return jobs, err
}

func (r *JobRepository) GetJobsByStatus(status string) ([]models.Job, error) {
	var jobs []models.Job
	err := r.DB.Where("status = ?", status).Preload("Tractor").Preload("Trailer").Preload("Driver").Preload("Customer").Find(&jobs).Error
	return jobs, err
}

func (r *JobRepository) ListJobs(offset, limit int) ([]models.Job, error) {
	var jobs []models.Job
	query := r.DB.Preload("Tractor").Preload("Trailer").Preload("Driver").Preload("Customer")
	if limit > 0 {
