...
| BUG-REG-012 | Trip form — Phân bổ nơi đổ dầu vs định mức cố định tuyến | Sửa chuyến OWN dùng tuyến đã được thêm định mức dầu cố định SAU khi chuyến tạo: tổng dầu chuyến tính theo Km (~331L) thay vì định mức cố định tuyến (378L); nhập 378L (+ supplement) vào phân bổ dầu → lưu bị 400 "Tổng phân bổ dầu (382 lít) phải bằng tổng dầu chuyến (338 lít)" | Tạo chuyến cho tuyến CHƯA có `fixed_fuel_allowance`; sau đó set tuyến `fixedFuelAllowance=378`; mở chuyến (mọi trạng thái, kể cả COMPLETED) → sửa → phân bổ 382L → bấm Lưu cập nhật | Lưu thành công; `trips.fuel_liters=382`, `fuel_fixed_allowance_applied=378`. Chuyến CÓ snapshot >0 (đã từng có fixed allowance khi tạo) giữ nguyên snapshot — không bị recost | `updateTripFigures` đọc `trip.fuelFixedAllowanceApplied` (snapshot=0) mà không re-fetch `routes.fixed_fuel_allowance` khi tuyến không đổi; chỉ fallback sống (live) cho `fuelConfig`/`roadConfig`, không cho `routes.fixedFuelAllowance`. Fix: pure helper `resolveFuelFixedAllowanceApplied(snapshot, live)` — snapshot >0 giữ nguyên (B3/D4 anchor), snapshot =0 dùng live route (vì 0 = "chưa snapshot", không phải "0 by intent"). Áp dụng cho MỌI trạng thái — user vẫn phải edit được chuyến COMPLETED/IN_TRANSIT/LOCKED | `route-fixed-fuel-allowance-helper.test.ts` (4 ca pure); `route-fixed-fuel-allowance-live-fallback.test.ts` (3 ca integration: CREATED 378L, AUTO 331L, snapshot>0 giữ); `customer-trip-229-live-repro.test.ts` (1 ca chính là trip 229 từ prod DB). Thủ công: mở lại chuyến Lai Châu demo, nhập 378L phân bổ → Lưu thành công |
